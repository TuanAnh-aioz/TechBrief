...
class NewsAggregator:
        ...
        try:
            ...
            for entry in feed.entries[:limit]:
                ...
                existing = db.query(ResearchArticle).filter(
                    ResearchArticle.url == entry.link
                ).first()
    ...
    async def fetch_from_dev_to(db: Session, limit: int = 10) -> List[ResearchArticle]:
        ...
        try:
            response = requests.get(
                f"{NewsAggregator.RSS_FEEDS['dev_to']}&limit={limit}",
                timeout=10
            )
            ...
            for item in data[:limit]:
                existing = db.query(ResearchArticle).filter(
                    ResearchArticle.url == item.get("url")
                ).first()
    ...
    async def process_articles_with_ai(db: Session, limit: int = 20) -> int:
        ...
        unprocessed = db.query(ResearchArticle).filter(
